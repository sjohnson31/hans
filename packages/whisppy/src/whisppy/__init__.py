...
class Transcriber:
    ...
    def transcribe(
        self, samples: NDArray[np.float32], initial_prompt: str, grammar_rule: str
    ) -> str:
        return _w.transcribe(
            self._ctx, samples, initial_prompt, self._grammar, grammar_rule
        )


@contextmanager
def transcriber(
    model_path: str, gbnf_grammar: str
) -> Generator[Transcriber, None, None]:
    try:
